...
export default function Home() {
  ...
  {produtos.map((produto, index) => (
    <div key={produto.title} className="flex flex-col items-center w-full sm:max-w-2xl">
      <div
        className={`hover:scale-105 transition delay-150 duration-300 ease-in-out flex place-items-center bg-[#fff1f1] border-gray-200 rounded-3xl shadow-[...] ${
          index === 2 ? 'max-w-[250px] max-h-[15rem]' : 'md:max-w-lg h-[16rem]'
        }`}
      >
        <Image
          className={`p-3 w-full object-cover ${
            index === 2 ? 'h-[15rem]' : 'h-[15rem]'
          }`}
          src={produto.src}
